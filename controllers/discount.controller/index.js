...
const fs = require("fs");
const { Discount, validate } = require("../../models/discount.model");
...
exports.delete = async (req, res) => {
  const id = req.params.id;
  try {
    Discount.findByIdAndRemove(id, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    Discount.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const result = await new Discount({
      ...req.body,
    }).save();
    res.status(201).send({

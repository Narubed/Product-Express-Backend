...
const fs = require("fs");
const { CartShopping, validate } = require("../../models/cart.shoppping.model");
...
exports.delete = async (req, res) => {
  const id = req.params.id;
  try {
    CartShopping.findByIdAndRemove(id, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    CartShopping.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
      .then((data) => {
        console.log(data);
        if (!data) {
          res.status(404).send({

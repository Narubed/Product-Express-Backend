  ...
  Query: {
    ...
    products: async (root, args, ctx, info) => {
      ...
      let products = demoData.products.map((product) => {
        return {
          ...product,
          price: getMinMaxPrice(product),
          discount: Math.round(getDiscount(product)),
        };
      });
      ...
      if (args.category) {
        ...
        let subCat = demoData.productCategories.find(
          ...
          catSlugs.forEach((cat) => {
            if (
              item.categories.findIndex(
                (itemCat) => itemCat.slug === cat || cat === "all"
              ) > -1
            )
              catFlag = true;
          });
        ...
        if (flag && (args.colors.length || args.sizes.length)) {
          flag = item.variants.find(
            (variant) =>
              (!args.colors.length ||
                (variant.color &&
                  args.colors.find((color) => color === variant.color.slug))) &&
              (!args.sizes.length ||
                (variant.size &&
                  args.sizes.find((size) => size === variant.size.slug)))
          );
        }
      ...
      if (args.onlyData) {
        ...
        const relatedProducts = products.filter((item) => {
          ...
          item.categories.forEach((cat) => {
            if (
              product.categories.find((findCat) => findCat.slug === cat.slug)
            ) {
              rFlag = true;
            }
          });
    ...
    specialProducts: async (root, args, ctx, info) => {
      ...
      args.topRated &&
        (results = {
          ...results,
          topRated: products
            .sort((itemA, itemB) => itemB.ratings - itemA.ratings)
            .slice(0, args.count),
        });
      args.latest &&
        (results = {
          ...results,
          latest: products.filter((item) => item.is_new).slice(0, args.count),
        });
      args.onSale &&
        (results = {
          ...results,
          onSale: products
            .filter((item) => isSaleProduct(item))
            .slice(0, args.count),
        });
      return results;
    },
    ...
    posts: async (root, args, ctx, info) => {
      ...
      if (args.categories) {
        args.categories.forEach(item, (index) => {
          counts[index + 1] = posts.filter((post) =>
            post.categories.find((cat) => cat.slug === args.item)
          ).length;
        });
      }
      if (args.category) {
        posts = posts.filter((post) =>
          post.categories.find(
            (cat) => args.category === "all" || cat.slug === args.category
          )
        );
      }
      return {
